...
export class NavbarComponent implements OnInit {
    ...
    ngOnInit() {
      this.roles = this.authService.getRol();
       //this.listTitles = ROUTES.filter(listTitle => listTitle);
        this.listTitles =JSON.parse(localStorage.getItem('listOpciones')).filter(listTitle => listTitle);
      ...
      this.router.events.pipe(
       ...
       /* this._router = this.router.events.filter(event => event instanceof NavigationEnd).subscribe((event: NavigationEnd) => {
        ...
        this.rolesService.listarApps().subscribe((data: any[]) => {
            this.aplicaciones = data;      
            this.selectedApp = this.aplicaciones[0].Id;
            localStorage.setItem("App",this.selectedApp as any);
         });
    ...
    onAppsChange(tr: { value: any; }) {
        console.log(tr);
        this.selectedApp = tr.value;
        localStorage.setItem("App",this.selectedApp as any);
