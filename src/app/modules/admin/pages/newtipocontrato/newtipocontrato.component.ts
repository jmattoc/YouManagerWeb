...
@Component({
  selector: 'app-newtipocontrato',
  templateUrl: './newtipocontrato.component.html',
  styleUrls: []
})
export class NewtipocontratoComponent implements OnInit {
 ...
 public showestado: boolean = true;
  ...
  constructor(
   ...
   // console.log('parametro',this.activatedRoute.snapshot.paramMap.get('id'));
    ...
    if(id != null){
      ...
      this.isedit = true;
      this.titulocategoria ='EDITAR TIPO CONTRATO';
      this.subtitulo ='Editar tipo contrato';
      this.btnguar ='Editar';
  ...
  chaneestadocel(e: any){
    ...
    console.log('BOOLEAN-estado', e.value);
  }

  async guardartipocontrato(){
    ...
    if(this.isedit){
     // console.log('idmaestrodetalle',this.idMaestroDetalle);
      ...
      this.adminservice.putupdatemaestrodetalle(dataedit).subscribe((res:  any) =>{
        ...
        this.router.navigate(["/administracion/tipocontrato"]);
      });
    }else{
      var data = {
        fkMaestro:21,
      ...
      this.adminservice.postguardartipocontrato(data).subscribe((res:  any) =>{
        this.bootstrapNotifyBarService.notifyWarning('Se creo correctamente el Tipo de Contrato.');
        this.router.navigate(["/administracion/tipocontrato"]);
