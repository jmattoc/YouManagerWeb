...
@Component({
  selector: 'app-newcentrocosto',
  templateUrl: './newcentrocosto.component.html',
  styleUrls: []
})
export class NewcentrocostoComponent implements OnInit {
  ...
  stamanewcodcentrocosto = new FormControl('');
  stamanenewnombcentrocosto = new FormControl('');
  public showregiscateg: boolean = true;
  /*ngmodel */
  public estadoint: number;
  public newcategoriatxt: string;
  public estadointtxt: boolean= true;
  public newcodcentrocosto: string;
  public newnombrecentrocosto: string;
 ...
 public titulocategoria: string = 'NUEVO CENTRO DE COSTO';
 public subtitulo: string = 'Crear centro de costo';
 ...
 public showestado: boolean = true;
  ...
  constructor(
    ...
    if(id != null){
      ...
      this.isedit = true;
      this.titulocategoria ='EDITAR CENTRO DE COSTO';
      this.subtitulo ='Editar centro de costo';
      this.btnguar ='Editar';
  ...
  ngOnInit(): void {
    ...
    this.formularionewcategoria = this.fb.group({
      ...
      stamanewcodcentrocosto: this.stamanewcodcentrocosto,
      stamanenewnombcentrocosto: this.stamanenewnombcentrocosto,
      fkMdEstado: [],
      stCodigo: [],
  ...
  obtenerdatosmaestrodetalle(id){
    this.adminservice.geteditaradmin(id).subscribe((res: any) => {
      //console.log('aaa', res);
      this.newnombrecentrocosto = res.stDescripcion;
      this.estadointtxt = res.blActivo;
      this.idMaestroDetalle = res.idMaestroDetalle;
      this.newcodcentrocosto = res.stNombre.split('-')[0];
     // console.log('slice', this.newcodcentrocosto);
     ...
     // console.log('idmaestrodetalle',this.idMaestroDetalle);
      var dataedit ={
        idMaestroDetalle: this.idMaestroDetalle,
         stDescripcion: this.newnombrecentrocosto,
        stNombre:this.newcodcentrocosto +' - '+ this.newnombrecentrocosto,
      ...
      this.adminservice.putupdatemaestrodetalle(dataedit).subscribe((res:  any) =>{
        ...
        this.router.navigate(["/administracion/centrocosto"]);
      });
    }else {
      var data = {
        fkMaestro:3,
      ...
      this.adminservice.postguardarcentrocosto(data).subscribe((res:  any) =>{
        this.bootstrapNotifyBarService.notifyWarning('Se creo el nuevo centro de costo.');
        this.router.navigate(["/administracion/centrocosto"]);
