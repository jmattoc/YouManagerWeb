...
describe('Reentrancy -', function () {
    ...
    it('Select()', function () {
        ...
        const expected = [].concat(...data_1.jsn.map(a => a.name));
        chai_1.assert.sameOrderedMembers(actual, expected);
        const actualReentrant = [...iterable];
        chai_1.assert.sameOrderedMembers(actualReentrant, expected);
    });
    it('ThenBy()', function () {
    ...
    it('GroupJoin()', function () {
        var iterable = linq_1.asEnumerable(data_1.msdn)
            ...
            return {
                Owner: person.Name,
                Pets: linq_1.asEnumerable(petCollection).Select(pet => pet.Name)
                    .ToArray()
            };
        });
        var iterator = iterable[Symbol.iterator]();
    ...
    it('GroupBy()', function () {
        ...
        const expected = new Map(data_1.pets.map(pet => [pet.Age, data_1.pets.filter(p => p.Age === pet.Age).length]));
        ...
        for (const [expectedAge, expectedLength] of expected) {
            ...
            chai_1.assert.isDefined(actualGroup, `Missing expected group for pet.Age with key ${expectedAge}`);
            chai_1.assert.equal(actualGroup.length, expectedLength, `Expected pet.Age ${expectedAge} with group of ${expectedLength} items but instead got ${actualGroup.length} items`);
        }
        // Reentrancy
        const actualReentrant = [...iterable[Symbol.iterator]()];
        for (const [expectedAge, expectedLength] of expected) {
            const actualGroup = actualReentrant.find(group => group.key === expectedAge);
            chai_1.assert.isDefined(actualGroup, `expected reentrant group for pet.Age with key ${expectedAge}`);
            chai_1.assert.equal(actualGroup.length, expectedLength, `Expected reentrant pet.Age ${expectedAge} with group of ${expectedLength} items but instead got ${actualGroup.length} items`);
        }
    });
    it('SelectMany()', function () {
        var iterable = linq_1.asEnumerable(data_1.jsn).SelectMany(a => a.ids);
        const actual = [...iterable];
        const expected = [].concat(...data_1.jsn.map(a => a.ids));
        chai_1.assert.sameOrderedMembers(actual, expected);
        const actualReentrant = [...iterable];
        chai_1.assert.sameOrderedMembers(actualReentrant, expected);
    });
    ...
    it('tzachshabtay', function () {
        var paths = ["1", "aa"];
        chai_1.assert.doesNotThrow(() => { linq_1.asEnumerable(paths).Where(p => p === "aa").Any(q => true); });
        chai_1.assert.doesNotThrow(() => { linq_1.asEnumerable(paths).Where(v => v === "aa").Any(); });
    });
});
