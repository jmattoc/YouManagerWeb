...
continue;set.add(value);yield value;}}
exports.UnionFast=UnionFast;function*Join(target,oKeySelect,transform,map){for(let value of target){let key=oKeySelect(value);if(key==null)
continue;let innerSet=map.get(key);if(!innerSet)
continue;for(let inner of innerSet){yield transform(value,inner);}}}
exports.Join=Join;function*GroupJoin(target,oKeySelect,transform,map){for(let value of target){let innerSet=undefined;let key=oKeySelect(value);innerSet=map.get(key);yield transform(value,innerSet);}}
...
return this;const compare=keySelect?equal?(a,b)=>equal(keySelect(a),keySelect(b)):(a,b)=>Constant.defCompare(keySelect(a),keySelect(b)):equal;const superEqual=this.comparer;const comparer=(!superEqual)?compare:(a,b)=>superEqual(a,b)||(this.reversed?-compare(a,b):compare(a,b));return new OrderedLinq(this._target,this._factory,undefined,comparer,this.reversed);}
ThenByDescending(keySelect,equal){if(!keySelect&&!equal)
return this;const compare=keySelect?equal?(a,b)=>equal(keySelect(a),keySelect(b)):(a,b)=>Constant.defCompare(keySelect(a),keySelect(b)):equal;const superEqual=this.comparer;const comparer=(!superEqual)?compare:(a,b)=>superEqual(a,b)||(this.reversed?compare(a,b):-compare(a,b));return new OrderedLinq(this._target,this._factory,undefined,comparer,this.reversed);}}},{"./generators":1,"./iterators":2,"./utilities":4}],4:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});exports.trueFn=()=>true;exports.selfFn=(o)=>o;exports.defGrouping=(a,b)=>{if(!b[exports.CONST_KEY]){b[exports.CONST_KEY]=a;}
...
return undefined;}
exports.getDefaultVal=getDefaultVal;function getKeyedMap(iterable,keySelector,selElement=exports.selfFn){let map=new Map();for(let value of iterable){let key=keySelector(value);let group=map.get(key);if(!group){group=[];map.set(key,group);}
group.push(selElement(value));}
return map;}
exports.getKeyedMap=getKeyedMap;function getKeyedMapFast(iterable,keySelector){let map=new Map();for(let value of iterable){let key=keySelector(value);let group=map.get(key);if(!group){group=[];map.set(key,group);}
group.push(value);}
return map;}
exports.getKeyedMapFast=getKeyedMapFast;function getKeys(iterable,keySelector){let set=new Set();if(keySelector){for(let value of iterable){let key=keySelector(value);set.add(key);}}
